...
const Contact = () => {
  ...
  // ⚡ SHOWCASE MODE: Set to `true` for your portfolio/preview site.
  // Set to `false` when your business opens and you want real emails.
  ...
  const handleSubmit = async (e) => {
    ...
    // 🎨 Handling for Portfolio Showcase Mode
    if (SHOWCASE_MODE) {
      setTimeout(() => {
        alert(`[Showcase Mode] Thank you, ${formData.name}! Your simulated message has been processed successfully. In production, this routes straight to the artisan studio.`);
        setFormData({ name: '', email: '', message: '' });
        setIsSending(false);
        navigate('/success', { state: { type: 'contact' } });
      }, 800); // Small realistic delay for UI feel
      return;
    }

    // 🌍 SECURE CLOUD ENDPOINT: Used once you open your store live
    const SECURE_CONTACT_API_URL = "https://formspree.io";

    try {
      const response = await fetch(SECURE_CONTACT_API_URL, {
        method: 'POST',
        headers: { 
