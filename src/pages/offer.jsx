...
export default function Offer() {
  ...
  const offersData = [
    { id: 'offer-1', title: 'Sacred Earth', image: offerImages[1], description: 'An exclusive curation of black and brown-yellow stone beads. Limited time special offer.' },
    { id: 'offer-2', title: 'Spirit Collection', image: offerImages[2], description: 'A premium selection featuring vibrant cherry-red stone beads. Limited time special offer.' }
  ];
  ...
  const handleAddToCart = (offer) => {
    ...
    addToCart({ ...offer, subcategory: 'Offers' }, chosenOptionKey);
    alert(`${offer.title} successfully added to your basket!`);
  };
          ...
          return (
            <div key={offer.id} className="offer-flat-card" style={isFullySoldOut ? { opacity: 0.45 } : {}}>
              <div className="image-section" style={{ position: 'relative' }}>
                ...
                {isFullySoldOut && (
                  <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'rgba(0,0,0,0.85)', color: '#ff4d00', padding: '0.4rem 0.8rem', borderRadius: '4px', fontSize: '1.1rem', fontWeight: 'bold', border: '1px solid #ff4d00', whiteSpace: 'nowrap' }}>
              ...
              <div className="info-section">
                ...
                <div className="actions-wrapper" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', width: '100%' }}>
                  ...
                  <div className="price-action-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', gap: '1rem' }}>
                    ...
                    >
                      {isFullySoldOut ? 'Sold Out' : isCurrentOptionInCart ? 'Unavailable' : 'Add To Cart'}
                    </Button>
                  </div>
                </div>
